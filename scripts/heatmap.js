...
fetch("https://api.tvmaze.com/shows/49/episodes")
    ...
    .then(function(data){
        ...
        const seasons = data.map((season, index) => {
            return{
        ...
        //Runs the function
        createMap(seasons, totalSeasons);
        createScales();
    });

//Creating the map
function createMap(data, totalSeasons){
    let HEIGHT = 600,
        WIDTH = 600,
        MARGIN = 80;
...
function createScales(){
    let xScale = d3.scaleLinear().domain([1, totalSeasons]).range([0, WIDTH]);
    svg.append("g")
